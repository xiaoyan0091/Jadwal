...
def format_jadwal_hari_ini():
    ...
    if jadwal_data["harian"][today]:
        ...
        msg += "❌ <i>Tidak ada jadwal donghua hari ini</i>\n"
    ...
    if jadwal_data["upcoming"]:
        for i, up in enumerate(jadwal_data["upcoming"], 1):
            # Format blockquote hijau seperti di foto
            msg += f'<blockquote expandable>{i}. <b>{up["judul"]}</b>\n'
            if up.get("season"):
                msg += f'[Season {up["season"]}]\n'
            msg += f'({up["hari"]}, {up["tanggal"]}) (<a href="{up["link"]}"><i>PV</i></a>)</blockquote>\n\n'
    else:
        msg += "<blockquote>Belum ada upcoming donghua dalam waktu dekat</blockquote>\n\n"
    
    # Footer dengan link Telegraph
    if jadwal_data.get("telegraph_url"):
        msg += f'<a href="{jadwal_data["telegraph_url"]}"><b>Jadwal Domgha Semua Hari</b></a>\n#botjadwal'
    else:
...
async def panel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channel_info = jadwal_data["channel_id"] if jadwal_data["channel_id"] else "❌ Belum diset"
    ...
    next_post = "Tidak ada" if not jadwal_data["auto_post_enabled"] else f"Setiap hari jam {jadwal_data['post_time']}"
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "setup_telegraph":
        ...
        await query.edit_message_text(
            "<b>📺 SET CHANNEL UNTUK AUTO POST</b>\n\n"
        ...
        context.user_data['waiting_input'] = 'channel'
        ...
        if jadwal_data["auto_post_enabled"]:
            if not jadwal_data["channel_id"]:
                await query.answer("❌ Set channel/group dulu!")
            ...
            try:
                ...
                # Gunakan timezone WIB untuk scheduling
                wib_time = time(hour, minute, tzinfo=WIB)
                context.job_queue.run_daily(
                    scheduled_daily_post,
                    time=wib_time,
                    name='daily_auto_post'
                )
                await query.answer(f"✅ Auto post AKTIF! Format seperti foto contoh setiap hari jam {jadwal_data['post_time']} WIB")
            except Exception as e:
        ...
        try:
            await context.bot.send_message(
                chat_id=jadwal_data["channel_id"],
                text=format_jadwal_hari_ini(),
                parse_mode='HTML',
                disable_web_page_preview=True
            )
            await query.answer("✅ Jadwal berhasil dikirim dengan format seperti foto!")
            await panel_refresh(query, context)
...
📝 Jadwal hari ini: <b>{jadwal_hari_ini} anime</b>
📈 Total minggu ini: <b>{total_minggu} anime</b>
📺 Channel/Group: <code>{channel_info}</code>
⏰ Jam auto post: <b>{time_info} WIB</b>
🤖 Status: <b>{auto_status}</b>
📰 Telegraph: <b>{telegraph_status}</b>
⏭️ Posting: <b>{next_post}</b>
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_for == 'tambah':
        ...
        try:
            ...
            jadwal_data["channel_id"] = text
            save_data()
            
            await update.message.reply_text(
                f"✅ <b>Channel/Group Berhasil Diset!</b>\n\n"
                f"📺 <b>Chat ID:</b> <code>{text}</code>\n"
                f"🔗 <b>Status:</b> Terhubung\n\n"
                f"<i>💡 Bot siap posting format seperti foto contoh!</i>", 
                parse_mode='HTML'
            )
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                f"<i>💡 Bot akan posting otomatis sesuai jam yang sudah diset!</i>\n"
...
async def scheduled_daily_post(context: ContextTypes.DEFAULT_TYPE):
    """Job harian - kirim dengan format seperti foto contoh dengan timezone WIB yang benar"""
    if not jadwal_data["auto_post_enabled"] or not jadwal_data["channel_id"]:
        return
    ...
    try:
        ...
        # Update Telegraph sebelum posting
        if jadwal_data.get("telegraph_token"):
            update_telegraph()
        ...
        try:
            ...
            await context.bot.send_message(
                OWNER_ID,
                f"✅ <b>Auto Post Berhasil!</b>\n\n"
                f"📅 <b>Hari:</b> {today}\n"
                f"📝 <b>Jadwal:</b> {status}\n"
                f"⏰ <b>Waktu:</b> {now_wib.strftime('%H:%M WIB')}\n"
                f"🎨 <b>Format:</b> Sama seperti foto contoh{telegraph_info}\n"
                f"📺 <b>Target:</b> <code>{jadwal_data['channel_id']}</code>",
                parse_mode='HTML'
            )
        except Exception as e:
...
def main():
    ...
    # Setup daily auto post jika sudah enabled dengan timezone WIB
    if jadwal_data.get("auto_post_enabled", False) and jadwal_data.get("channel_id"):
        try:
            hour, minute = map(int, jadwal_data["post_time"].split(":"))
            # Gunakan timezone WIB untuk job scheduling
            wib_time = time(hour, minute, tzinfo=WIB)
            ...
            print(f"⏰ Daily auto post restored: {jadwal_data['post_time']} WIB")
        except Exception as e:
            logger.error(f"Failed to restore daily auto post: {e}")
            print(f"⚠️ Failed to restore daily auto post: {e}")
    
    print(f"🚀 Bot Started!")
    print(f"📺 Channel: {jadwal_data.get('channel_id', 'Not Set')}")
    print(f"⏰ Post Time: {jadwal_data.get('post_time', '06:00')} WIB") 
